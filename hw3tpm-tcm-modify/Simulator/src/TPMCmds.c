...
#include <include/bsl/bsl_log.h>
#include <include/bsl/bsl_errno.h>
#define PURPOSE                                                                 \
...
{
    return 0 == strcmp(opt, optFull) || (optShort && opt[0] == optShort[0] && opt[1] == 0) || (dashed && opt[0] == '-' && 0 == strcmp(opt + 1, optFull));
}
...
static bool CmdLineParser_IsOptPresent(const char *optFull, const char *optShort)
{
    ...
    for (i = 0, curArgBit = 1; i < s_Argc; ++i, curArgBit <<= 1)
    {
        ...
        if ((s_ArgsMask & curArgBit) && opt && (0 == strcmp(opt, optFull) || ((opt[0] == '/' || opt[0] == '-') && CmdLineParser_IsOpt(opt + 1, optFull, optShort, opt[0] == '-'))))
        {
...
{
    char version[HITLS_VERSION_LEN] = {0};
    uint32_t versionLen = HITLS_VERSION_LEN;
    
    printf("Crypto implementation information:\n");
    
    // 获取 OpenHiTLS 版本信息
    if (BSL_LOG_GetVersion(version, &versionLen) == BSL_SUCCESS) {
        printf(" Symmetric: OpenHiTLS (%s)\n", version);
        printf(" Hashing: OpenHiTLS (%s)\n", version);
        printf(" Math: OpenHiTLS (%s)\n", version);
    } else {
        // 如果获取版本失败，使用默认的版本字符串
        printf(" Symmetric: OpenHiTLS (%s)\n", OPENHITLS_VERSION_S);
        printf(" Hashing: OpenHiTLS (%s)\n", OPENHITLS_VERSION_S);
...
int main(int argc, char *argv[])
{
    ...
    {
        if (CmdLineParser_IsOptPresent("?", "?") || CmdLineParser_IsOptPresent("help", "h"))
        {
            Usage(argv[0]);
        }

...
{
    printf("[DBG] _TPM_Init enter\n");
    g_powerWasLost = g_powerWasLost | _plat__WasPowerLost();
    printf("[DBG] _TPM_Init powerWasLost=%d\n", g_powerWasLost);
    ...
    g_nvOk = NvPowerOn();
    printf("[DBG] _TPM_Init NvPowerOn -> %d\n", g_nvOk);

    // Initialize cryptographic functions
    g_inFailureMode = (g_nvOk == FALSE) || (CryptInit() == FALSE);
    printf("[DBG] _TPM_Init CryptInit complete g_inFailureMode=%d\n", g_inFailureMode);
    if(!g_inFailureMode)
    {
        // Load the persistent data
        NvReadPersistent();
        printf("[DBG] _TPM_Init NvReadPersistent done\n");

        // Load the orderly data (clock and DRBG state).
        // If this is not done here, things break
        NvRead(&go, NV_ORDERLY_DATA, sizeof(go));
        printf("[DBG] _TPM_Init NvRead orderly data done\n");

        // Start clock. Need to do this after NV has been restored.
        TimePowerOn();
        printf("[DBG] _TPM_Init TimePowerOn done\n");
    }
    else
    {
        printf("[DBG] _TPM_Init entering failure mode\n");
    }
    return;
}
...
#define ALLOWED_SEQUENCES (SUBJECT_PUBLIC_KEY_REF - SIGNATURE_REF)
    ...
    if ((ctx.tag != (ASN1_CONSTRUCTED_SEQUENCE)) || ((ctx.offset + length) != in->partialCertificate.t.size))
    ...
    // number of elements.
    if ((ctx.offset != ctx.size) || (countOfSequences < 3) || (countOfSequences > 4) || (certTBS[EXTENSIONS_REF].buf == NULL))
        return TPM_RCS_VALUE + RC_CertifyX509_partialCertificate;
    ...
    // See if it can
    if ((countOfSequences == 3) && (X509AddSigningAlgorithm(NULL, signKey, &in->inScheme) == 0))
    ...
    if (result != TPM_RC_SUCCESS)
        ...
        return result + ((result == TPM_RCS_ATTRIBUTES) ? RC_CertifyX509_objectHandle

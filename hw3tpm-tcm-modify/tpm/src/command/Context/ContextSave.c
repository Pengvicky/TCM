...
)
{
    ...
    integritySize = sizeof(integrity.t.size) + CryptHashGetDigestSize(CONTEXT_INTEGRITY_HASH_ALG);
    ...
    case TPM_HT_TRANSIENT:
    {
        ...
        UINT16 objectSize = ObjectIsSequence(object) ? sizeof(HASH_OBJECT)
                                                     : sizeof(OBJECT);
        outObject = (ANY_OBJECT_BUFFER *)(out->context.contextBlob.t.buffer + integritySize + fingerprintSize);
        ...
        if (object->publicArea.type == TPM_ALG_RSA && !(object->attributes.publicOnly))
            CryptRsaLoadPrivateExponent(&object->publicArea, &object->sensitive);
#endif
        // Make sure things fit
        pAssert(out->context.contextBlob.t.size <= sizeof(out->context.contextBlob.t.buffer));
    ...
    case TPM_HT_POLICY_SESSION:
    {
        ...
        pAssert(sizeof(*session) <= sizeof(out->context.contextBlob.t.buffer) - integritySize - fingerprintSize);
        MemoryCopy(
    ...
    pAssert(sizeof(out->context.sequence) <= sizeof(out->context.contextBlob.t.buffer) - integritySize);

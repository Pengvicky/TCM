...
#  if ALG_ECDSA
#    define TPM_ECC_PWCT_SIG_ALG TPM_ALG_ECDSA
#    define TPM_ECC_PWCT_SIG_ALG_VALUE ALG_ECDSA_VALUE
#    define TpmEcc_PwctSign(bnR, bnS, E, bnD, digest, rand) \
    TpmEcc_SignEcdsa(bnR, bnS, E, bnD, digest, rand)
#    define TpmEcc_PwctValidate(bnR, bnS, E, ecQ, digest) \
    (TpmEcc_ValidateSignatureEcdsa(bnR, bnS, E, ecQ, digest) == TPM_RC_SUCCESS)
#  elif ALG_SM2
#    define TPM_ECC_PWCT_SIG_ALG TPM_ALG_SM2
#    define TPM_ECC_PWCT_SIG_ALG_VALUE ALG_SM2_VALUE
#    define TpmEcc_PwctSign(bnR, bnS, E, bnD, digest, rand) \
    TpmEcc_SignEcSm2(bnR, bnS, E, bnD, digest, rand)
#    define TpmEcc_PwctValidate(bnR, bnS, E, ecQ, digest) \
    (TpmEcc_ValidateSignatureEcSm2(bnR, bnS, E, ecQ, digest) == TPM_RC_SUCCESS)
#  else
#    define TPM_ECC_PWCT_SIG_ALG TPM_ALG_NULL
#    define TPM_ECC_PWCT_SIG_ALG_VALUE ALG_NULL_VALUE
#    define TpmEcc_PwctSign(...) TPM_RC_FAILURE
#    define TpmEcc_PwctValidate(...) FALSE
#  endif
//** Functions
...
)
{
    ...
    TPM_RC retVal;
    //
#  if TPM_ECC_PWCT_SIG_ALG_VALUE != ALG_NULL_VALUE
    TPM_DO_SELF_TEST(TPM_ECC_PWCT_SIG_ALG);  // Ensure selected ECC signer is tested
#  endif
...
#  if FIPS_COMPLIANT && TPM_ECC_PWCT_SIG_ALG_VALUE != ALG_NULL_VALUE
    ...
    if(OK && IS_ATTRIBUTE(publicArea->objectAttributes, TPMA_OBJECT, sign))
    {
        ...
        TpmEcc_PwctSign(bnT, bnS, E, bnD, &digest, NULL);
        // and make sure that we can validate the signature
        OK = TpmEcc_PwctValidate(bnT, bnS, E, ecQ, &digest);
    }
#  endif

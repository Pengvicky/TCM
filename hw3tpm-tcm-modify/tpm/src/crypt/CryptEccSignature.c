                               ...
                               RAND_STATE*         rand)
{
    ...
    TPMS_SIGNATURE_ECC* sigPtr = NULL; // Selected R/S container
    //
...
#if ALG_ECDSA
        case TPM_ALG_ECDSA:
            sigPtr = &signature->signature.ecdsa;
            retVal = TpmEcc_SignEcdsa(bnR, bnS, E, bnD, digest, rand);
            break;
#endif
#if ALG_ECDAA
        case TPM_ALG_ECDAA:
            sigPtr = &signature->signature.ecdaa;
            // ECDAA returns R in the TPM2B container directly
            retVal = TpmEcc_SignEcdaa(&sigPtr->signatureR, // TPM2B_ECC_PARAMETER R
                                      bnS,
                                      E,
                                      bnD,
                                      digest,
                                      ...
                                      signKey,
                                      rand);
            // R provided via sigPtr->signatureR (bnR not used)
            bnR = NULL;
            break;
#endif
#if ALG_ECSCHNORR
        case TPM_ALG_ECSCHNORR:
            sigPtr = &signature->signature.ecschnorr;
            retVal = TpmEcc_SignEcSchnorr(bnR,
                                          bnS,
                                          E,
                                          bnD,
                                          digest,
...
#if ALG_SM2
        case TPM_ALG_SM2:
            sigPtr = &signature->signature.sm2;
    ...
    // If signature generation worked, convert the results (R/S) for schemes using bnR/bnS.
    if(retVal == TPM_RC_SUCCESS && sigPtr != NULL)
    {
        // Initialize size fields
        sigPtr->signatureR.t.size = sizeof(sigPtr->signatureR.t.buffer);
        sigPtr->signatureS.t.size = sizeof(sigPtr->signatureS.t.buffer);
        NUMBYTES orderBytes = (NUMBYTES)BITS_TO_BYTES(
            ExtMath_SizeInBits(ExtEcc_CurveGetOrder(ExtEcc_CurveGetCurveId(E))));
        if(bnR != NULL)
            TpmMath_IntTo2B(bnR, &sigPtr->signatureR.b, orderBytes);
        if(bnS != NULL)
            TpmMath_IntTo2B(bnS, &sigPtr->signatureS.b, orderBytes);
    }
Exit:
    CRYPT_CURVE_FREE(E);
    return retVal;
...
)
{
    ...
    TPM_RC               retVal;
    TPMS_SIGNATURE_ECC*  sigPtr = NULL;
    ...
    // Select the appropriate signature container based on algorithm
    switch(signature->sigAlg)
    {
#if ALG_ECDSA
        case TPM_ALG_ECDSA:
    ...
    // Convert R and S from the selected container
    if(sigPtr == NULL)
        ERROR_EXIT(TPM_RC_SCHEME);
    TpmMath_IntFrom2B(bnR, &sigPtr->signatureR.b);
    TpmMath_IntFrom2B(bnS, &sigPtr->signatureS.b);

    // r and s must be in the proper range
...
#if ALG_ECDAA
        case TPM_ALG_ECDAA:
            // ECDAA uses bnS and sigPtr->signatureR already in container (bnR may be NULL)
            retVal = TpmEcc_ValidateSignatureEcdaa(&sigPtr->signatureR,
                                                   bnS,
                                                   sigPtr->hash,
                                                   E,
                                                   ecQ,
...
#if ALG_ECSCHNORR
        case TPM_ALG_ECSCHNORR:
            retVal = TpmEcc_ValidateSignatureEcSchnorr(bnR,
                                                       bnS,

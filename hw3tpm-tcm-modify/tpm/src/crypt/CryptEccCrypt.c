...
#  endif
    ...
    /* MGF1 is disabled in TCM profile; gracefully reject if ALG_MGF1 == NO */
#if !ALG_MGF1
    ERROR_EXIT(TPM_RC_SCHEME);
#else
    ...
    /* Generate mask only if MGF1 enabled */
#if ALG_MGF1
    c2->t.size = CryptMGF_KDF(plainText->t.size,
                              ...
                              1);
#else
    c2->t.size = plainText->t.size; /* passthrough for disabled MGF1 */
    MemoryCopy(c2->t.buffer, plainText->t.buffer, plainText->t.size);
#endif
...
)
{
    ...
    // Generate the mask
    /* Recover mask if MGF1 enabled; otherwise passthrough */
#if ALG_MGF1
    plainText->t.size = CryptMGF_KDF(c2->t.size,
                                     ...
                                     1);
#else
    plainText->t.size = c2->t.size;
    MemoryCopy(plainText->t.buffer, c2->t.buffer, c2->t.size);
#endif

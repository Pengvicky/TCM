...
)
{
    ...
    if (pcr < IMPLEMENTATION_PCR)
    {
        for (i = 0; i < gp.pcrAllocated.count; i++)
        {
            ...
            {
                if (((gp.pcrAllocated.pcrSelections[i].pcrSelect[pcr / 8]) & (1 << (pcr % 8))) != 0)
                    allocated = TRUE;
                else
                    allocated = FALSE;
                break;
...
{
    BOOL selected;
    selected = (pcr < IMPLEMENTATION_PCR && ((selection->pcrSelect[pcr / 8]) & (1 << (pcr % 8))) != 0);
    return selected;
}
//*** FilterPcr()
        ...
        for (j = 0; j < gp.pcrAllocated.count; j++)
        {
            ...
            if (pcrData != NULL)
            {
                ...
                else // PCR was not restored by state save
                {
                    ...
                    pAssert_BOOL(pcrInitialResult == TPM_RC_SUCCESS || pcrInitialResult == TPM_RC_PCR);

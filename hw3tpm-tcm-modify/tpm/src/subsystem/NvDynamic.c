...
#error "NV_MEMORY_SIZE probably isn't large enough"
    ...
    return ((reserved < remainBytes) && (size <= remainBytes) && (size + reserved <= remainBytes));
}
//*** NvWriteNvListEnd()
...
)
{
    ...
    if (IS_ATTRIBUTE(nvAttributes, TPMA_NV, ORDERLY))
    {
        ...
        pAssert(ramAddr != 0 && (size <= ((NV_RAM_HEADER *)ramAddr)->size - sizeof(NV_RAM_HEADER) - offset));
        MemoryCopy(data, ramAddr + sizeof(NV_RAM_HEADER) + offset, size);
    }
    else
    {
        // Validate that read falls within range of the index
        pAssert(offset <= nvIndex->publicArea.dataSize && size <= (nvIndex->publicArea.dataSize - offset));
...
)
{
    ...
    if (!IS_ATTRIBUTE(nvIndex->publicArea.attributes, TPMA_NV, WRITTEN))
    {
        ...
        if (!IS_ATTRIBUTE(nvIndex->publicArea.attributes, TPMA_NV, ORDERLY))
        {
            ...
            // index.
            if (IsNvOrdinaryIndex(nvIndex->publicArea.attributes) && (nvIndex->publicArea.dataSize > size))
...
)
{
    ...
    // as well
    if (IS_ATTRIBUTE(publicArea->attributes, TPMA_NV, ORDERLY) && !NvRamTestSpaceIndex(publicArea->dataSize))
        return TPM_RC_NV_SPACE;
...
)
{
    ...
    if (nvIndex != NULL)
    {
        ...
        if (IsNvCounterIndex(nvIndex->publicArea.attributes) && IS_ATTRIBUTE(nvIndex->publicArea.attributes, TPMA_NV, WRITTEN))
...
)
{
    ...
    while ((currentAddr = NvNext(&iter, &entityHandle)) != 0)
    {
        ...
        else if (HandleGetType(entityHandle) == TPM_HT_PERSISTENT)
        {
            ...
            //
            NvRead(&attributes,
                   (UINT32)(currentAddr + sizeof(TPM_HANDLE) + offsetof(OBJECT, attributes)),
                   sizeof(OBJECT_ATTRIBUTES));
            // If the evict object belongs to the hierarchy to be flushed...
            if ((hierarchy == TPM_RH_PLATFORM && attributes.ppsHierarchy == SET) || (hierarchy == TPM_RH_OWNER && attributes.spsHierarchy == SET) || (hierarchy == TPM_RH_ENDORSEMENT && attributes.epsHierarchy == SET))
            {
...
NvSetGlobalLock(void)
{
    ...
    while ((currentAddr = NvNextIndex(NULL, &iter)) != 0)
    {
        ...
        if (!IS_ATTRIBUTE(attributes, TPMA_NV, ORDERLY) && IS_ATTRIBUTE(attributes, TPMA_NV, GLOBALLOCK))
        {
...
)
{
    ...
    if (!IsNvCounterIndex(attributes))
    {
        if (IS_ATTRIBUTE(attributes, TPMA_NV, CLEAR_STCLEAR) || (IS_ATTRIBUTE(attributes, TPMA_NV, ORDERLY) && (type == SU_RESET)))
    ...
    if (!IS_ATTRIBUTE(attributes, TPMA_NV, WRITTEN) || !IS_ATTRIBUTE(attributes, TPMA_NV, WRITEDEFINE))
...
NvCapGetCounterAvail(void)
{
    ...
    if (availNVSpace / NV_INDEX_COUNTER_SIZE > availRAMSpace / NV_RAM_INDEX_COUNTER_SIZE)

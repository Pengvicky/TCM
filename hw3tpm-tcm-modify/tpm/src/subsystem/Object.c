...
{
    pAssert(object != NULL);
    return (object->attributes.hmacSeq == SET || object->attributes.hashSeq == SET || object->attributes.eventSeq == SET);
}
//*** HandleToObject()
...
)
{
    ...
    else
    {
        ...
        object->attributes.stClear =
            (IS_ATTRIBUTE(objectAttributes, TPMA_OBJECT, stClear) || (parent->attributes.stClear == SET));
        ...
        // is external
        object->attributes.temporary = parent->attributes.temporary || object->attributes.external;
    ...
    else
    {
        ...
        if (IS_ATTRIBUTE(objectAttributes, TPMA_OBJECT, restricted) && !object->attributes.publicOnly && IS_ATTRIBUTE(objectAttributes, TPMA_OBJECT, decrypt) && object->publicArea.nameAlg != TPM_ALG_NULL)
        {
...
)
{
    ...
    if ((parent == NULL) || ((parent != NULL) && !IS_ATTRIBUTE(
...
)
{
    ...
    if (CryptMacStart(&hmacObject->state.hmacState,
                      ...
                      hashAlg,
                      &keyObject->sensitive.sensitive.any.b) == 0)
...
)
{
    ...
    if (HierarchyNormalizeHandle(object->hierarchy) == TPM_RH_ENDORSEMENT && gc.ehEnable == CLEAR && GetCommandCode(commandIndex) != TPM_CC_EvictControl)
        return TPM_RC_HANDLE;
    return result;
}
//*** ObjectComputeName()
...
)
{
    ...
    //
    return (publicArea != NULL && IS_ATTRIBUTE(publicArea->objectAttributes, TPMA_OBJECT, restricted) && IS_ATTRIBUTE(publicArea->objectAttributes, TPMA_OBJECT, decrypt) && !IS_ATTRIBUTE(publicArea->objectAttributes, TPMA_OBJECT, sign) && (object->publicArea.type == TPM_ALG_RSA || object->publicArea.type == TPM_ALG_ECC));
}
//*** ObjectCapGetLoaded()

...
)
{
    ...
    if (!(Unmarshal16(&header.tag, &buffer, &size) && Unmarshal32(&header.size, &buffer, &size) && Unmarshal32(&header.code, &buffer, &size)))
        goto FailureModeReturn;
    ...
    case TPM_CC_GetCapability:
        ...
        if (header.size != (10 + (3 * sizeof(UINT32)))
            ...
            || !Unmarshal32(&capability, &buffer, &size) || capability != TPM_CAP_TPM_PROPERTIES || !Unmarshal32(&pt, &buffer, &size) || !Unmarshal32(&count, &buffer, &size))
            goto FailureModeReturn;
        if (count > 0)
            count = 1;

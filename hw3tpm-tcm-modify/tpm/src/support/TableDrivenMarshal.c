...
#define _MB_ (*buffer)
    ...
    if ((*size -= bytes) >= 0)
    {
        ...
        {
            *((UINT32 *)target) =
                (UINT32)((((((_MB_[0] << 8) | _MB_[1]) << 8) | _MB_[2]) << 8) | _MB_[3]);
...
)
{
            ...
            else
            {
                ...
                for (i = tmt->singles - 1; i >= 0; i--)
                {
                    ...
                    if (val == check[i])
                    {
                        ...
                        if ((HAS_BITS & tmt->modifiers) && (!IS_BIT_SET32(i, &(check[tmt->singles]))))
    ...
    case MIN_MAX_MTYPE:
    {
                ...
                UnmarshalInteger(sel->modifiers, target, buffer, size, &val)))
        {
            ...
            {
                val -= *check;
                if ((val > check[1]) || ((mmt->modifiers & HAS_BITS) && !IS_BIT_SET32(val, &check[2])))
                    result = (TPM_RC)(mmt->errorCode);
            }
        }
        break;

    ...
    for (i = 0; i < command->handleNum; i++)
    {
        ...
        case TPM_HT_PERMANENT:
            ...
            default:
                ...
                if (((TPM_RH)handle >= TPM_RH_AUTH_00) && ((TPM_RH)handle <= TPM_RH_AUTH_FF))
                    // if the implementation has a manufacturer-specific value
                    result = TPM_RC_VALUE;
                else

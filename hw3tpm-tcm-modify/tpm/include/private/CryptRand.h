...
#if ALG_SM4
#define DRBG_KEY_SIZE_BITS SM4_MAX_KEY_SIZE_BITS
#define DRBG_IV_SIZE_BITS (SM4_MAX_BLOCK_SIZE * 8)
#define DRBG_ALGORITHM TPM_ALG_SM4
#define DRBG_ENCRYPT_SETUP(key, keySizeInBits, schedule) \
    TpmCryptSetEncryptKeySM4(key, keySizeInBits, schedule)
#define DRBG_ENCRYPT(keySchedule, in, out) \
    TpmCryptEncryptSM4(SWIZZLE(keySchedule, in, out))
#elif ALG_AES
...
#else
#error "DRBG requires either SM4 or AES to be enabled"
#endif
...
#if RSA_INSTRUMENT
#define PRIME_INDEX(x) ((x) == 512 ? 0 : (x) == 1024 ? 1 \
                                                     : 2)
